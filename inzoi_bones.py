"""
Definición de huesos estándar para el avatar de PureQ
Utilizado para validación y filtrado durante la importación
"""

PureQ_AVATAR_BONES = [
    # Root / Hips
    "root", "pelvis",
    
    # Spine
    "spine_01", "spine_02", "spine_03", "spine_04", "spine_05",
    
    # Neck / Head
    "neck_01", "neck_02", "head",
    
    # Arms (Left)
    "clavicle_l", "upperarm_l", "lowerarm_l", "hand_l",
    "upperarm_twist_01_l", "upperarm_twist_02_l",
    "lowerarm_twist_01_l", "lowerarm_twist_02_l",
    
    # Arms (Right)
    "clavicle_r", "upperarm_r", "lowerarm_r", "hand_r",
    "upperarm_twist_01_r", "upperarm_twist_02_r",
    "lowerarm_twist_01_r", "lowerarm_twist_02_r",
    
    # Fingers (Left)
    "thumb_01_l", "thumb_02_l", "thumb_03_l",
    "index_01_l", "index_02_l", "index_03_l",
    "middle_01_l", "middle_02_l", "middle_03_l",
    "ring_01_l", "ring_02_l", "ring_03_l",
    "pinky_01_l", "pinky_02_l", "pinky_03_l",
    
    # Fingers (Right)
    "thumb_01_r", "thumb_02_r", "thumb_03_r",
    "index_01_r", "index_02_r", "index_03_r",
    "middle_01_r", "middle_02_r", "middle_03_r",
    "ring_01_r", "ring_02_r", "ring_03_r",
    "pinky_01_r", "pinky_02_r", "pinky_03_r",
    
    # Legs (Left)
    "thigh_l", "calf_l", "foot_l", "ball_l", "bigtoe_01_l",
    "thigh_twist_01_l", "thigh_twist_02_l",
    "calf_twist_01_l", "calf_twist_02_l",
    
    # Legs (Right)
    "thigh_r", "calf_r", "foot_r", "ball_r", "bigtoe_01_r",
    "thigh_twist_01_r", "thigh_twist_02_r",
    "calf_twist_01_r", "calf_twist_02_r",
    
    # Facial / Special (Basic set)
    "facial_root", "jaw", "tongue_01", "tongue_02", "eye_l", "eye_r",
    
    # Extra bones often found in PureQ rigs
    "breast_l", "breast_r",
    "buttock_l", "buttock_r",
    
    # IK Targets (sometimes present)
    "ik_hand_root", "ik_hand_gun", "ik_hand_l", "ik_hand_r",
    "ik_foot_root", "ik_foot_l", "ik_foot_r"
]
